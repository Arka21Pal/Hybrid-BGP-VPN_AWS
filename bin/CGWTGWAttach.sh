...
# Basic variables:
    ...
    profile=""
    region_name=""
    stack_name="CGWTGWAttach"

# ---------------------
# Template(s) to run

    template="../src/CGWTGW_attachINFRA.yaml"
...
# Retain logs
    ...
    if [  "${retain_log}" = 1 ]; then
        ...
        aws cloudformation describe-stack-events --stack-name "${stack_name}" --region "${region_name}" --profile "${profile}" >> cgw_tgwattachments_logfile
    fi
}
